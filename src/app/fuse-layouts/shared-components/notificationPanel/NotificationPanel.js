...
function NotificationPanel(props) {
  ...
  useEffect(() => {
    ...
    // setTimeout(
    //   () => createNotification({ message: 'Vehicle is successfully added!', options: { variant: 'success' } }),
    //   4000
    // );

    // setTimeout(() => createNotification({ message: 'Vehicle is removed!', options: { variant: 'error' } }), 6000);
    ...
    // setTimeout(
    //   () => createNotification({ message: 'Vehicle already exists.', options: { variant: 'info' } }),
    //   10000
    // );
  }, [dispatch]);
